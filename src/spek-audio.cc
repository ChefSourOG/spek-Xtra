...
public:
    AudioFileImpl(
        ...
        int audio_stream, const std::string& codec_name, const std::string& format_name,
        int bit_rate, int sample_rate, int bits_per_sample, int streams, int channels,
        double duration
    );
    ...
    std::string get_format_name() const override { return this->format_name; }
...
std::unique_ptr<AudioFile> Audio::open(const std::string& file_name, int stream)
{
    ...
    if (!error) {
        ...
        if (format_context->iformat && format_context->iformat->name) {
            format_name = format_context->iformat->name;
        }
    ...
    return std::unique_ptr<AudioFile>(new AudioFileImpl(
        ...
        audio_stream, codec_name, format_name, bit_rate, sample_rate,
...
AudioFileImpl::AudioFileImpl(
    ...
    audio_stream(audio_stream), codec_name(codec_name), format_name(format_name),
    bit_rate(bit_rate),
