...
std::string spek_pipeline_format_name(const struct spek_pipeline *pipeline)
{
    return pipeline->file->get_format_name();
}
    ...
    if(prefs.get_show_detailed_description()) {
        if (pipeline->file->get_error() == AudioError::OK) {
            ...
            case WINDOW_RECTANGULAR:
                window_function_name = std::string("Rectangular");
                break;
            default:
                assert(false);
            }
...
int spek_pipeline_bits_per_sample(const struct spek_pipeline *pipeline)
{
    return pipeline->file->get_bits_per_sample();
}
